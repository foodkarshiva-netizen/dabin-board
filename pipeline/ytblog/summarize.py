...
- 본문 문단(intro, 구간 summary, key_takeaways.text, conclusion)에서 가장 중요한 구절을 문단마다 1~2곳만 **이렇게** 별표 두 개로 감쌉니다. 블로그에서 형광펜 굵은 글씨로 표시됩니다. 문장 전체가 아니라 핵심 구절(6~20자)만, 남발 금지. card_points·short·seo·image_caption 에는 쓰지 않습니다.
- 중학생도 이해할 수 있는 쉬운 말로 씁니다. 전문용어는 피하고, 꼭 써야 하면 괄호 안에 한 줄로 풀어 씁니다. 문장은 짧게 끊습니다.
...
def synthesize(llm, settings: Settings, system: list[dict], sections: list[Section],
    ...
    prompt = (
        ...
        "그대로 글에 실립니다. 본문(~해요 체)과 달리 혼잣말 메모 어투로 씁니다: 음슴체('~라고 생각함', '~임')와 평서체('~라고 생각한다'), 되묻는 말('~지 않나?')을 섞어 3~4문장.\n"
